...
def main():
	...
	if not op.exists('./dti_FA.nii.gz'):
	    dti_params = dti.fit_dti(config['data_file'], config['data_bval'], config['data_bvec'], out_dir='.')
	else:
	...
	mapping = reg.syn_register_dwi(config['data_file'], gtab)
	reg.write_mapping(mapping, './mapping.nii.gz')


	print("Segmenting fiber groups...")
	fiber_groups = seg.segment(config['data_file'],
		                   config['data_bval'],
		                   config['data_bvec'],
		                   streamlines,
		                   bundles,
